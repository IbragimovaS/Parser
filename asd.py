...
def get_channels():
    ...
    cursor = con.cursor()
    sql = 'select channel_id from tl_media_channels where source_id = 1 order by id limit(30) offset(30)'
...
def insert_into_db_test(posts, owner_id, group_name):
    ...
    for post in posts:
        ...
        object_id = post['id']
        cursor.execute(
            'select 1 from tl_media_data_vk where object_id = \'{object_id}\' and channel_id=\'\'\'{channel_id}\'\'\''.format(
                object_id=object_id, channel_id=owner_id))
        ...
        if one_row is not None:
            ...
            sql_update = 'update tl_media_data_vk set likes = {likes}, comments = {comments}, reposts={reposts}, views = {views} where object_id= \'{object_id}\' and channel_id=\'\'\'{channel_id}\'\'\''.format(
                likes=likes, comments=comments, reposts=reposts, views=views, object_id=object_id, channel_id=owner_id)
            ...
            cursor.execute(sql_history_test, (object_id, inserted_date, date, channel_id, likes, comments, views, reposts, caption, text, url_attachment[:255], url_channel, source_id))
    ...
    for comment in comments:
        ...
        if one_row is not None:
            ...
            sql_update = "update tl_media_comments_vk set comment_likes = {likes}, author_name =\'{author_name}\', author_url =\'{author_url}\', city =\'{city}\', bdate =\'{bdate}\', channel_id=\'\'{channel_id}\'\' where comment_id=\'{comment_id}\' and object_id={object_id}".format(
                likes=likes, author_name=author_name, author_url=author_url, city=city, bdate=bdate,
                comment_id=comment_id, channel_id=channel_id, object_id=object_id)
            print(sql_update)
            cursor.execute(sql_update)
        else:
...
access_token = '3bdbb97d00d778774ff2cdbfa56626b5b2ac883c1d7f92820a0088720c58c607914cc2abe134812eed8ba'
